...
from utils import n_prefix_to_zoom_level, project_x_y_to_coordinate, string_to_number
...
if __name__ == "__main__":
    ...
    geojson_folder = Path("output") / "country"
    ...
    for n_prefix, countries in data.items():
        ...
        for prefix, country in tqdm(countries.items()):
            ...
            polygon_points = isbn_position_range_to_polygon(min_position, max_position)

            coordinates = [
                project_x_y_to_coordinate(int(x), int(y)) for (x, y) in polygon_points
            ]
        ...
        label_features_filepath = geojson_folder / f"country_{n_prefix}_labels.geojson"
